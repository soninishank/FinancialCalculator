...
function computeStepUpSchedule({ initialSIP, stepUpPercent, annualRate, years }) {
...
export default function StepUpSIP({ currency, setCurrency }) {
  ...
  const yearlyRows = useMemo(
    () => computeStepUpSchedule({ initialSIP: Number(initialSIP), stepUpPercent: Number(stepUpPercent), annualRate: Number(annualRate), years: Number(years) }),
    [initialSIP, stepUpPercent, annualRate, years]
  );
  ...
  function handleExport() {
    ...
    const rows = yearlyRows.map((r) => [
      `Year ${r.year}`, Math.round(r.totalInvested), Math.round(r.sipInvested), `${r.stepUpAppliedPercent}%`, Math.round(r.growth), Math.round(r.overallValue),
    ]);
